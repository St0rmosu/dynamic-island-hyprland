...
FocusScope {
    ...
    property real transitionDuration: boundedReal(userConfig.wallpaperTransitionDuration, 3, 0, 120)
    ...
    readonly property string cacheRoot: localPath(StandardPaths.writableLocation(StandardPaths.GenericCacheLocation)) + "/quickshell/dynamic_island/wallpaper-picker"
    ...
    readonly property real spacing: slotW * 1.2
    ...
    function releaseResources() {
        ...
        // Do not kill customApplyProcess so apply-wallpaper.sh completes in background
        pendingThumbnails = [];
        pendingThumbnailKeys = ({
        });
        thumbnailInFlight = false;
    ...
    function startNextThumbnail() {
        ...
        SystemServices.generateWallpaperThumbnail(next.sourcePath, next.cachePath, root.cacheRoot, root.thumbnailWidth, root.thumbnailHeight, root.thumbnailQuality);
    }
    ...
    Process {
        ...
        command: ["python3", "-c", root.applyScript, wallpaperPath, targetPath, transitionType, String(root.transitionStep), String(root.transitionDuration), String(root.transitionFps), String(root.transitionAngle), root.transitionPosition, root.transitionBezier, root.transitionWave, root.transitionInvertY ? "true" : "false", root.pywalEnabled ? "true" : "false"]
    ...
    Process {
        ...
        command: ["bash", "-c", root.homeDir + "/.scripts/apply-wallpaper.sh \"" + wallpaperPath + "\""]
        onExited: function(exitCode) {
            running = false;
            if (exitCode === 0)
                root.wallpaperApplySucceeded(wallpaperPath);
        ...
        stdout: SplitParser {
            onRead: (data) => {
                return console.log("[wallpaper stdout]", data);
            }
        }

        stderr: SplitParser {
            onRead: (data) => {
                return console.warn("[wallpaper stderr]", data);
            }
        }
    ...
    Column {
        ...
        Item {
            ...
            Rectangle {
                ...
                radius: 10
                color: Qt.rgba(1, 1, 1, searchBar.expanded ? 0.06 : (iconMouse.containsMouse ? 0.1 : 0.06))
        ...
        Item {
            ...
            PathView {
                ...
                delegate: Item {
                    ...
                    property real sc: isCurrent ? 1 : onPath ? root.sideScale : 0
                    property real op: isCurrent ? 1 : onPath ? 0.65 : 0
