...
Item {
    ...
    readonly property string effectiveSource: {
        if (cacheLoader.item && cacheLoader.item.effectiveSource !== "")
            return cacheLoader.item.effectiveSource;
        if (wallpaperPath === "") return "";
        return wallpaperPath.startsWith("file://") ? wallpaperPath : ("file://" + encodeURI(wallpaperPath));
    }
