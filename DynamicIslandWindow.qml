...
PanelWindow {
    ...
    readonly property bool overviewDataReady: overviewLoader.item ? !!overviewLoader.item.overviewDataReady : true
    ...
    readonly property bool monitorFocused: {
        compositorRevision;
        return compositorIsNiri ? CompositorBackend.isOutputFocused(screenOutputName) : (hyprlandIntegration ? hyprlandIntegration.monitorFocused : false);
    }
    ...
    readonly property real connectivityDetailWindowHeight: root.anyConnectivityDetailMounted ? Math.ceil(effectiveIslandTopMargin + root.connectivityDetailHeight + 12) : 0
    readonly property real overviewWindowHeight: root.overviewVisible ? Math.ceil(effectiveIslandTopMargin + root.overviewCapsuleHeight + 8) : 0
    readonly property real musicFloatingIslandHeight: musicFloatingIsland && musicFloatingIsland.visible ? Math.ceil(musicFloatingIsland.y + musicFloatingIsland.height + 12) : 0
    readonly property real headphonesFloatingIslandHeight: headphonesFloatingIsland && headphonesFloatingIsland.visible ? Math.ceil(headphonesFloatingIsland.y + headphonesFloatingIsland.height + 12) : 0
    readonly property real callFloatingIslandHeight: callFloatingIsland && callFloatingIsland.visible ? Math.ceil(callFloatingIsland.y + callFloatingIsland.height + 12) : 0
    readonly property real requestedWindowHeight: Math.max(root.notificationCenterWindowHeight, root.capsuleWindowHeight, root.connectivityDetailWindowHeight, root.overviewWindowHeight, Math.ceil(root.controlCenterWindowHeight), root.musicFloatingIslandHeight, root.headphonesFloatingIslandHeight, root.callFloatingIslandHeight)
    ...
    readonly property real effectiveIslandWidth: {
        if (dynamicConfig && dynamicConfig.islandWidth > 0)
            return dynamicConfig.islandWidth;
    ...
    readonly property real effectiveIslandHeight: {
        if (dynamicConfig && dynamicConfig.islandHeight > 0)
            return dynamicConfig.islandHeight;
    ...
    readonly property real effectiveIslandTopMargin: {
        if (dynamicConfig && dynamicConfig.islandTopMargin >= 0)
            return dynamicConfig.islandTopMargin;
    ...
    readonly property real effectiveIslandBackgroundOpacity: {
        if (dynamicConfig && dynamicConfig.islandBackgroundOpacity !== undefined)
            return dynamicConfig.islandBackgroundOpacity;
    ...
    readonly property real effectiveIslandPositionX: {
        if (dynamicConfig && dynamicConfig.islandPositionX !== undefined)
            return dynamicConfig.islandPositionX;
    ...
    readonly property bool satelliteBalancingAllowedState: {
        if (!islandContainer)
            return false;
    ...
    readonly property bool anySatelliteExpanded: {
        return Boolean((musicFloatingIsland && musicFloatingIsland.isExpanded) || (headphonesFloatingIsland && headphonesFloatingIsland.isExpanded) || (callFloatingIsland && callFloatingIsland.isExpanded));
    }
    readonly property bool satelliteBalancingActive: autoBalanceSatellites && !root.overviewVisible && satelliteBalancingAllowedState && !anySatelliteExpanded
    readonly property real satelliteLeftExtent: {
        if (!satelliteBalancingActive)
            return 0;

        var ext = 0;
        try {
            var musicActive = Boolean(musicFloatingIsland && musicFloatingIsland.visible && !musicFloatingIsland.isExpanded && musicFloatingIsland.opacity > 0.05);
            if (musicActive) {
                var mw = (Number(musicFloatingIsland.width) || 38) + 7;
                ext = Math.max(ext, mw);
            }
            var shelfActive = Boolean(fileShelfBubble && fileShelfBubble.visible && fileShelfBubble.opacity > 0.05);
            if (shelfActive) {
                var fw = (Number(fileShelfBubble.width) || 36) + 8;
                ext = Math.max(ext, fw);
            }
        } catch (e) {
        }
    ...
    readonly property real satelliteRightExtent: {
        if (!satelliteBalancingActive)
            return 0;

        var ext = 0;
        try {
            var hpActive = Boolean(headphonesFloatingIsland && headphonesFloatingIsland.visible && !headphonesFloatingIsland.isExpanded && headphonesFloatingIsland.opacity > 0.05);
            var callActive = Boolean(callFloatingIsland && callFloatingIsland.visible && !callFloatingIsland.isExpanded && callFloatingIsland.opacity > 0.05);
            var hpW = hpActive ? (Number(headphonesFloatingIsland.width) || 38) : 0;
            var callW = callActive ? (Number(callFloatingIsland.width) || 38) : 0;
            if (hpActive && callActive)
                ext = hpW + 7 + callW + 7;
            else if (hpActive)
                ext = hpW + 7;
            else if (callActive)
                ext = callW + 7;
            var timerActive = Boolean(timerBubble && timerBubble.visible && timerBubble.reveal > 0.05);
            if (timerActive) {
                var timerW = (Number(timerBubble.width) || 34) + 8;
                ext = Math.max(ext, timerW);
            }
        } catch (e) {
        }
        return ext;
    }
    readonly property real satelliteBalanceOffset: (satelliteLeftExtent - satelliteRightExtent) / 2
    property real animatedBalanceOffset: satelliteBalanceOffset
    ...
    readonly property real effectiveIslandCornerRadius: {
        if (dynamicConfig && dynamicConfig.islandCornerRadius > 0)
            return dynamicConfig.islandCornerRadius;
    ...
    readonly property bool autoHideRestingState: islandContainer.islandState === "normal" || islandContainer.islandState === "custom" || islandContainer.islandState === "lyrics"
    readonly property bool autoHideCanHideNow: autoHideEnabled && autoHideRestingState && !root.overviewVisible && !root.connectivityPromptActive && !root.anyConnectivityDetailMounted
    readonly property bool autoHideMustShow: !autoHideRestingState || root.overviewVisible || root.connectivityPromptActive || root.anyConnectivityDetailMounted
    readonly property bool autoHideTargetVisible: autoHideMustShow || (!autoHideForcedHidden && (!autoHideEnabled || autoHideVisible))
    readonly property bool autoHideSuppressesTransientReveal: (autoHideEnabled || autoHideForcedHidden) && !autoHideTargetVisible
    property real autoHideProgress: autoHideTargetVisible ? 1 : 0
    readonly property bool exclusiveZoneTargetActive: (!autoHideEnabled && autoHideTargetVisible) || (autoHideRevealSource === "edge" && autoHideTargetVisible) || islandContainer.notificationLayerVisible || islandContainer.reloadLayerVisible
    ...
    readonly property real autoHideRevealX: Math.max(0, Math.min(root.width - autoHideRevealWidth, root.width * root.effectiveIslandPositionX / 100 - autoHideRevealWidth / 2))
    readonly property real topGestureInputX: autoHideEnabled ? autoHideRevealX : 0
    readonly property real topGestureInputWidth: topGestureInputActive ? (autoHideEnabled ? autoHideRevealWidth : root.width) : 0
    readonly property real topGestureInputHeight: topGestureInputActive ? (autoHideEnabled ? autoHideRevealHeight : root.baseExclusiveZone) : 0
    ...
    readonly property real controlCenterWindowHeight: islandContainer.controlCenterLayerVisible ? root.effectiveIslandTopMargin + (controlCenterLoader.item ? controlCenterLoader.item.controlCenterPreferredHeight : 450) + 60 : 0
    readonly property real notificationCenterWindowHeight: islandContainer.notificationCenterLayerVisible ? root.effectiveIslandTopMargin + (notificationCenterLoader.item ? notificationCenterLoader.item.contentHeight : 400) + 6 : 0
    ...
    function refreshWallpaperSources() {
        if (walWallpaperFile)
            walWallpaperFile.reload();

        if (pywalColors)
            pywalColors.reload();

        if (currentWallpaperFile)
    ...
    function setAutoHideRevealSource(source) {
        ...
        autoHideRevealSource = nextSource === "edge" || nextSource === "state" || nextSource === "manual" ? nextSource : "manual";
    }
    ...
    function closeScreenSharePicker() {
        islandContainer.closeScreenSharePicker();
    }
    ...
    function focusScreenSharePicker() {
        islandContainer.forceActiveFocus();
        if (screenSharePickerLoader.item) {
            screenSharePickerLoader.item.forceActiveFocus();
            if (screenSharePickerLoader.item.grabKeyboardFocus)
                screenSharePickerLoader.item.grabKeyboardFocus();
    ...
    function dragCarriesFiles(dragEvent) {
        ...
        return formats.indexOf("text/uri-list") >= 0 || formats.indexOf("x-special/gnome-copied-files") >= 0;
    }
    ...
    WlrLayershell.layer: islandContainer.wallpaperPickerLayerVisible || islandContainer.applicationLauncherLayerVisible || islandContainer.clipboardLayerVisible || islandContainer.settingsAppLayerVisible || islandContainer.fileShelfLayerVisible || islandContainer.polkitLayerVisible || islandContainer.screenSharePickerLayerVisible || islandContainer.powerMenuLayerVisible || islandContainer.notificationLayerVisible ? WlrLayer.Overlay : WlrLayer.Top
    WlrLayershell.keyboardFocus: {
        if (islandContainer.polkitLayerVisible || islandContainer.screenSharePickerLayerVisible || islandContainer.controlCenterLayerVisible || islandContainer.powerMenuLayerVisible || islandContainer.wallpaperPickerLayerVisible || islandContainer.applicationLauncherLayerVisible || islandContainer.clipboardLayerVisible || islandContainer.settingsAppLayerVisible)
        ...
        onLoaded: {
            try {
                ...
                if (idata) {
                    ...
                    if (idata.green)
                        greenHex = idata.green;

                    if (idata.yellow)
                        yellowHex = idata.yellow;

                    if (idata.syntax_type || idata.syntax_keyword)
                        purpleHex = idata.syntax_type || idata.syntax_keyword;

                    if (idata.syntax_func)
                        blueHex = idata.syntax_func;

                    console.log("[DynamicIsland] Loaded iris colors! accent:", accentHex, "fg:", fgHex);
                }
            } catch (e) {
    ...
    FileView {
        ...
        onLoaded: {
            try {
                ...
                if (data.special) {
                    if (data.special.background)
                        background = data.special.background;

                    if (data.special.foreground)
    ...
    FocusScope {
        ...
        property var notificationHistoryModel
        ...
        readonly property real timerProgress: timerActive && timerTotalSeconds > 0 ? Math.max(0, Math.min(1, timerRemainingSeconds / timerTotalSeconds)) : 0
        readonly property bool timerBubbleWanted: (timerActive && timerRemainingSeconds > 0 || timerCompletionAnimating) && !root.overviewVisible && (islandState === "normal" || islandState === "lyrics" || islandState === "custom")
        readonly property bool fileShelfBubbleWanted: FileShelf.count > 0 && !root.overviewVisible && (islandState === "normal" || islandState === "lyrics" || islandState === "custom")
        readonly property bool fileShelfCanAutoOpen: !root.overviewVisible && (islandState === "normal" || islandState === "lyrics" || islandState === "custom")
        readonly property bool blocksTransientSplit: islandState === "expanded" || islandState === "bluetooth_expanded" || islandState === "control_center" || islandState === "power_menu" || islandState === "notification" || islandState === "reload" || islandState === "wallpaper_picker" || islandState === "application_launcher" || islandState === "clipboard" || islandState === "settings_app" || islandState === "file_shelf" || islandState === "polkit" || islandState === "charging" || islandState === "silent_ring" || islandState === "discord_call"
        ...
        readonly property bool screenSharePickerLayerVisible: !root.overviewVisible && islandState === "screen_share_picker"
        property string screenShareFifoPath: ""
        ...
        function showNotificationCapsule(appName, summary, body, icon, customColor) {
            ...
            if (islandState === "settings_app") {
                const floatingExpanded = (musicFloatingIsland && musicFloatingIsland.isExpanded) || (headphonesFloatingIsland && headphonesFloatingIsland.isExpanded);
                if (floatingExpanded) {
        ...
        function showDiscordCallCapsule(callerName, subtitle, avatarUrl) {
            if (root.overviewVisible || islandState === "polkit")
                return ;
        ...
        function suppressCapsuleClick(cancelPreparedOverview) {
            if (cancelPreparedOverview === undefined)
                cancelPreparedOverview = false;
        ...
        function smartRestoreState() {
            if (screenShareFifoPath !== "") {
                Quickshell.execDetached(["bash", "-c", "echo 'cancel' > '" + screenShareFifoPath + "'"]);
                screenShareFifoPath = "";
            }
        ...
        function showBluetoothExpanded(device) {
            if (!device || root.overviewVisible || islandState === "control_center" || islandState === "notification" || islandState === "file_shelf")
                return ;
        ...
        function showScreenSharePicker(fifoPath) {
            cancelSideSwipeSettle();
            abortSideTransientMode();
            clearTransientCapsule();
            screenShareFifoPath = fifoPath || "";
            islandState = "screen_share_picker";
            mainCapsule.displayedWidth = mainCapsule.baseTargetWidth;
            stopAutoHideTimer();
            root.showAutoHiddenIsland("state");
            root.focusScreenSharePicker();
        }

        function closeScreenSharePicker() {
            if (islandState === "screen_share_picker")
                smartRestoreState();
        ...
        anchors.fill: parent
        focus: controlCenterLayerVisible || wallpaperPickerLayerVisible || applicationLauncherLayerVisible || clipboardLayerVisible || settingsAppLayerVisible || fileShelfLayerVisible || polkitLayerVisible || expandedPlayerKeyboardFocusRequested || (root.monitorFocused && (root.overviewVisible || root.connectivityPromptActive))
        ...
        onCurrentTrackChanged: {
            const disableAutoExpand = (localUserConfigFile.parsedData && localUserConfigFile.parsedData.disableAutoExpandOnTrackChange !== undefined) ? Boolean(localUserConfigFile.parsedData.disableAutoExpandOnTrackChange) : (userConfig.disableAutoExpandOnTrackChange !== undefined ? Boolean(userConfig.disableAutoExpandOnTrackChange) : true);
            ...
            if (currentTrack !== "" && islandState !== "control_center" && islandState !== "notification" && islandState !== "bluetooth_expanded" && islandState !== "file_shelf") {
                if (root.autoHideSuppressesTransientReveal)
                    return ;
        ...
        WifiConnectionTracker {
            ...
            onWifiDisconnected: function(ssid) {
                islandContainer.showNotificationCapsule("Wi-Fi", "Wi-Fi disconnesso", ssid ? ("da " + ssid) : "", "", "#ff9f0a");
            }
        }
        ...
        IslandSystemState {
            ...
            lyricsCavaActive: islandContainer.lyricsSwipeVisible && islandContainer.rightSwipeProgress > 0.001
        ...
        Timer {
            ...
            repeat: false
            onTriggered: {
                if (capsuleMouseArea.containsMouse || mainCapsuleHoverHandler.hovered || (trayFloatingIsland && trayFloatingIsland.visible && trayFloatingIsland.hovered) || (trayFloatingIsland && trayFloatingIsland.visible && trayFloatingIsland.menuVisible))
                    return ;

                if (!islandContainer.hoverExpandedActive)
                    return ;

                if (islandContainer.islandState === "settings_app" || islandContainer.islandState === "clipboard" || islandContainer.islandState === "wallpaper_picker" || islandContainer.islandState === "application_launcher" || islandContainer.islandState === "file_shelf") {
        ...
        Item {
            ...
            Rectangle {
                ...
                opacity: Math.min(1, Math.max(0, (topAnchorProxy.width - 50) / 180))
        ...
        Rectangle {
            ...
            property color outlineColor: root.overviewContentVisible ? root.overviewCapsuleBorderColor : (notificationHistorySurface ? "#1affffff" : StyleTokens.clearBlack)
            ...
            readonly property real baseTargetWidth: {
                ...
                case "notification":
                    ...
                    return Math.max(notificationLoader.item.minimumWidth, Math.min(root.width - 48, notificationLoader.item.maximumWidth, notificationLoader.item.preferredWidth));
                case "reload":
            ...
            readonly property real targetHeight: {
                ...
                case "bluetooth_expanded":
                    return 165;
                case "notification":
                    return notificationLoader.item ? Math.max(56, notificationLoader.item.preferredHeight) : 56;
                case "reload":
            ...
            function sideSwipeWidthForProgress(progressValue) {
                if (progressValue < 0)
                    return root.effectiveIslandWidth + (islandContainer.customCapsuleWidth - root.effectiveIslandWidth) * islandContainer.clamp01(-progressValue);

                if (progressValue > 0)
                    return root.effectiveIslandWidth + (islandContainer.lyricsCapsuleWidth - root.effectiveIslandWidth) * islandContainer.clamp01(progressValue);
            ...
            Loader {
                ...
                visible: active
                onLoaded: {
                    if (islandContainer.openTimerPageWhenExpanded && item && item.openTimerPage) {
                        item.openTimerPage();
            ...
            Loader {
                ...
                sourceComponent: Component {
                    ControlCenterLayer {
                        ...
                        nightLightEnabled: root.shellRootController && root.shellRootController.nightLightEnabled !== undefined ? root.shellRootController.nightLightEnabled : false
            ...
            Loader {
                ...
                sourceComponent: Component {
                    WallpaperPickerLayer {
                        ...
                        onWallpaperApplied: (filePath) => {
                            return root.wallpaperPickerActiveWallpaper = filePath;
                        }
                        onWallpaperApplySucceeded: (filePath) => {
                            return root.handleWallpaperApplySucceeded(filePath);
                        }
                        onCloseRequested: islandContainer.smartRestoreState()
                    }
            ...
            Loader {
                ...
                anchors.fill: parent
                active: islandContainer.screenSharePickerLayerVisible
                asynchronous: false
                visible: islandContainer.screenSharePickerLayerVisible
                focus: true
                onLoaded: root.focusScreenSharePicker()

                sourceComponent: Component {
                    ScreenSharePickerLayer {
                        accentColor: pywalColors.accent
                        iconFontFamily: root.iconFontFamily
                        textFontFamily: root.textFontFamily
                        heroFontFamily: root.heroFontFamily
                        monitorName: root.screenOutputName !== "" ? root.screenOutputName : "eDP-1"
                        showCondition: islandContainer.screenSharePickerLayerVisible
                        onSelectionMade: function(choice) {
                            if (choice && islandContainer.screenShareFifoPath !== "") {
                                Quickshell.execDetached(["bash", "-c", "echo '" + choice + "' > '" + islandContainer.screenShareFifoPath + "'"]);
                                islandContainer.screenShareFifoPath = "";
                            }
                            islandContainer.smartRestoreState();
                        }
                        onCloseRequested: {
        ...
        notificationHistoryModel: ListModel {
        }
    ...
    Behavior on animatedBalanceOffset {
        NumberAnimation {
            duration: 380
            easing.type: Easing.OutQuint
        }
