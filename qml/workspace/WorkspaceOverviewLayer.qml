...
Item {
    ...
    readonly property int effectiveActiveWorkspaceId: {
        const monWs = monitor && monitor.activeWorkspace ? monitor.activeWorkspace : null
        let id = monWs ? (monWs.name ? parseInt(monWs.name) : (monWs.id > 0 ? monWs.id : undefined)) : undefined
        if (id === undefined || isNaN(id) || id <= 0) {
            const dataWs = hyprlandData && hyprlandData.activeWorkspace ? hyprlandData.activeWorkspace : null
            id = dataWs ? (dataWs.name ? parseInt(dataWs.name) : (dataWs.id > 0 ? dataWs.id : 1)) : 1
        }
    ...
    function resolveWindowWsId(wd, defaultWs) {
        if (!wd || !wd.workspace) return defaultWs !== undefined ? defaultWs : -1;
        const ws = wd.workspace;
        if (typeof ws === "number") return ws > 0 ? ws : (defaultWs !== undefined ? defaultWs : -1);
        ...
        return defaultWs !== undefined ? defaultWs : -1;
    }
    ...
    function windowAddressAtPoint(px, py) {
        ...
        for (let i = 0; i < windowToplevels.length; i++) {
            ...
            const wsId = hint && hint.workspace !== undefined ? hint.workspace : root.resolveWindowWsId(wd, -1)
    ...
    function clearMatchedWindowMoveHints() {
        ...
        for (const k in hints) {
            ...
            const wm = h.workspace === undefined || (wd.workspace && root.resolveWindowWsId(wd, -1) === h.workspace)
    ...
    function refreshToplevels() {
        ...
        for (let i = 0; i < toplevelValues.length; i++) {
            ...
            const addr = normalizeToplevelAddress(tlv)
            if (addr === "0x0" || addr === "0x" || addr === "") {
                hasPending = true
                continue
            }
            const wd = ba[addr] || null
            const wsId = root.resolveWindowWsId(wd, -1)
            if (wsId > start && wsId <= end) { next.push(tlv); sig += addr+"\x1e" }
        }
        if (hasPending && showCondition) {
            refreshTimer.restart()
        }
                                    ...
                                    Item {
                                        ...
                                        Repeater {
                                            ...
                                            delegate: WorkspaceOverviewWindow {
                                                ...
                                                readonly property int wsId: mh&&mh.workspace!==undefined ? mh.workspace : root.resolveWindowWsId(vwd, -1)
            ...
            Item {
                ...
                Repeater {
                    ...
                    delegate: WorkspaceOverviewWindow {
                        ...
                        readonly property var mh: root.windowMoveHint(addr)
                        readonly property int wsId: mh&&mh.workspace!==undefined ? mh.workspace : root.resolveWindowWsId(windowData, -1)
                        readonly property var ph: mh&&mh.x!==undefined&&mh.y!==undefined ? mh : null
                        ...
                        MouseArea {
                            ...
                            onPressed: (mouse) => {
                                ...
                                root.draggingFromWorkspace = root.resolveWindowWsId(dragTile.windowData, -1)
                            ...
                            onReleased: {
                                ...
                                const currentTileWs = root.resolveWindowWsId(dragTile.windowData, -1)
                                const moveWs = targetWs!==-1 && dragTile.windowData && dragTile.windowData.workspace && targetWs!==currentTileWs
