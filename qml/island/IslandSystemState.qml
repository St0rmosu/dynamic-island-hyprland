...
Item {
    ...
    signal chargingAlertRequested(bool isCharging, int capacity)
    signal lowBatteryAlertRequested(int capacity)
    signal muteAlertRequested(bool isMuted)
    ...
    Connections {
        ...
        function onVolumeChanged(volPercentage, isMuted) {
            ...
            if (muteToggled) {
                root.muteAlertRequested(isMuted);
                return;
            }
        ...
        function onBatteryChanged(capacity, statusString) {
            ...
            if (root._lastChargeStatus !== "" && root._lastChargeStatus !== statusString) {
                if (statusString === "Charging") {
                    root.chargingAlertRequested(true, capacity);
                } else if (statusString === "Discharging") {
                    root.chargingAlertRequested(false, capacity);
                }
            }
            root._lastChargeStatus = statusString;

            // Low Battery Alert: Trigger alert at <= 20% and <= 10% when discharging
            if (!root.isCharging && capacity > 0) {
                if (capacity <= 20 && (root._lastLowBatteryAlertCapacity > 20 || root._lastLowBatteryAlertCapacity === -1)) {
                    root._lastLowBatteryAlertCapacity = capacity;
                    root.lowBatteryAlertRequested(capacity);
                } else if (capacity <= 10 && root._lastLowBatteryAlertCapacity > 10) {
                    root._lastLowBatteryAlertCapacity = capacity;
                    root.lowBatteryAlertRequested(capacity);
                } else if (capacity > 25) {
                    root._lastLowBatteryAlertCapacity = -1;
                }
